...
"   python 0_train.py --data_choose 2 --model_choose 1 --train_adv '../dataX2/SeekThermal_resnet18_Train_pgd.h5py' --test_adv '../dataX2/SeekThermal_resnet18_Test_pgd.h5py' --save_path_ori '../modelF2/SeekThermal_resnet18_pgd.pth' --save_path_adv '../modelF3/SeekThermal_resnet18_pgd.pth'   "
...
train_iter = data.get_iter(path=opt.train_adv, batch_size=opt.batch_size, shuffle=True)  # 训练集是对抗样本训练集
test_iter = data.get_iter(path=opt.test_adv, batch_size=opt.batch_size, shuffle=False)  # 测试集是对抗样本测试集
...
torch.save(net.state_dict(), opt.save_path_adv)
